...
from utils import load_file_from_cwd, save_file_to_cwd, NpEncoder
import json
...
class Runner:
    ...
    def play(self):
        ...
        for seed in range(20):
            ...
            num_of_solution = len(solutions)
            finished_count = 0
            while finished_count < num_of_solution:
                action = self.infence_action(obs=obs, round=seed)
                action_sequence = []
                print(red("{0:-^50}".format("Environment start")))
                print(red(f"Solutions: {solutions}"))
                while not (terminated or truncated):
                    ...
                    action = self.infence_action(
                        obs=obs, rewards=reward, terminated=terminated, truncated=truncated, round=seed)
                    if terminated:
                        seq = "".join(map(str, action_sequence))
                        for i in range(len(solutions)):
                            solution = solutions[i]
                            if all([c in seq for c in solution]):
                                print(
                                    "Congratulations! You have finished the task. The solution is: ", seq)
                                finished_count += 1
    ...
    def infence_action(self, insight=None, knowledge=None, obs=None, rewards=None, terminated=None, truncated=None, round=None, action=None):
        ...
        if insight is None:
            ...
            insight = self.insighter.generate_insight(history.to_json())
        self.actioner.set_insight(insight)
        action = self.actioner.action(
            obs, rewards, terminated, truncated, round, action)
        return action
    ...
    def beysian_update(self):
        ...
        # from actioner
        history = self.actioner.get_history().to_json()
        insight = self.actioner.get_insight()
        success_try = json.dumps(
            self.actioner.get_success_try(), indent=2, cls=NpEncoder)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--mode', type=str, default='play',
                        help='function of the program, train, test ,play')
