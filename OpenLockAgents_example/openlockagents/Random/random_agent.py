...
from openlockagents.Random.model import DiscretePolicy
from openlockagents.Random.model import Value
from openlockagents.Random.core import update_params
from openlockagents.Random.utils.replay_memory import Memory
...
class RandomAgent(Agent):
    def __init__(self, env, state_size, action_size, params, require_log=True):
        ...
        :param params
        """
        super(RandomAgent, self).__init__("Random", params, env)
        if require_log:
            super(RandomAgent, self).setup_subject(human=False, project_src=params["src_dir"])

        self.params = params
        self.action_limit = params['train_action_limit']
    ...
    def run_trial_random(
        ...
        while not self.determine_trial_finished(attempt_limit):
            ...
            while not done:
                ...
                action_idx = self.act(state, step, train=True)
    ...
    def act(self, s, step, train=True):
        """
