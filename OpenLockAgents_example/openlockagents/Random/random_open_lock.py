...
from openlockagents.Random.random_agent import RandomAgent
...
def main():
    ...
    if len(sys.argv) < 2:
        ...
        # params = PARAMS['CE4']
        # params = PARAMS['CC4']
        params["reward_mode"] = 'negative_immovable_unique_solutions'
    else:
        setting = sys.argv[1]
    ...
    # dummy agent
    agent = A2CAgent(env, 1, 1, params, require_log=False)
    ...
    agent = A2CAgent(env, state_size, action_size, params)
    load_path = (
...
def Fool(str_scenario: str, gpuid: int, reward_mode: str='negative_immovable_unique_solutions', num_iter: int=100, attempt_limit: int=700,
         load_path: str=None):
    torch.set_default_tensor_type("torch.DoubleTensor")
    # Set up environment
    params = {
    ...
    # dummy agent
    agent = RandomAgent(env, 1, 1, params, require_log=False)
    trial_selected = agent.setup_trial(
        scenario_name=params["train_scenario_name"],
        action_limit=params["train_action_limit"],
        attempt_limit=params["train_attempt_limit"],
    ...
    state_size = agent.env.observation_space.multi_discrete.shape[0]
    action_size = len(env.action_space)
    agent = RandomAgent(env, state_size, action_size, params)
    save_path = os.path.join(
        params["data_dir"],
        "3rd_model_log/random-{}-{}-{}".format(
            transfer_tag if transfer_tag else params["train_scenario_name"],
            params["reward_mode"],
            agent.subject_id,
        ),
...
if __name__ == "__main__":
    # main()
    Fool('CC3-CC4', 0, 'negative_immovable_unique_solutions', 3, 700, None)
    #main()
