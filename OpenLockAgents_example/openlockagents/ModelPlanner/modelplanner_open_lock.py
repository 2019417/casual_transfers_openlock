...
from openlockagents.ModelPlanner.model_planner import ModelPlanner
...
def create_reward_fig():
    ...
    # plt.ion()
    fig = plt.figure()
    fig.set_size_inches(30, 10) #(20, 5)
    # plt.pause(0.0001)
    return fig


def modelplanner_learner(str_scenario: str, gpuid: int, reward_mode: str='negative_immovable_unique_solutions', num_iter: int=100, attempt_limit: int=700,
         load_path: str=None):
    ...
    # dummy agent
    agent = ModelPlanner(env, 1, 1, params, require_log=False)
    ...
    agent = ModelPlanner(env, state_size, action_size, params)
    save_path = os.path.join(
        params["data_dir"],
        "model_log/modelplanner-{}-{}-{}".format(
    ...
    agent.type_tag = "{}-{}-modelplanner".format(
        transfer_tag if transfer_tag else params["train_scenario_name"],
        params["reward_mode"],
    )
    ...
    for iter_num in tqdm(range(params["num_training_iters"])):
        for trial_num in range(0, len(possible_trials)):
            ...
            #simulated_memory.sample()
            batch = agent.memory.sample()
            
            print("update with bs:{}".format(len(batch.state)))
            agent.update(batch, iter_num, params["train_scenario_name"], trial_num)
            agent.memory.clear()
    ...
    if params["test_scenario_name"] == "CE4" or params["test_scenario_name"] == "CC4":
        ...
        for iter_num in tqdm(range(params["num_training_iters"])):
            ...
            print("update with bs:{}".format(len(batch.state)))
            agent.update(batch, iter_num, params["test_scenario_name"], 0)
            agent.memory.clear()
            #agent.simulated_memory.clear()
            print(f'after update:{len(agent.memory)}')
...
if __name__ == "__main__":
    # main()
    modelplanner_learner('CC3-CC4', 0, 'negative_immovable_unique_solutions', 2, 700, None)
    #main()
