...
from openlockagents.A2C_modelbased.model import World
from openlockagents.A2C_modelbased.core import update_params
from openlockagents.A2C_modelbased.utils.replay_memory import Memory


class A2CModelAgent(Agent):
    def __init__(self, env, state_size, action_size, params, require_log=True):
        ...
        :param params
        """
        super(A2CModelAgent, self).__init__("A2C", params, env)
        if require_log:
            super(A2CModelAgent, self).setup_subject(human=False, project_src=params["src_dir"])
    ...
    def _build_model(self):
        ...
        self.optimizer_world = torch.optim.Adam(self.world.parametesr(), lr = self.lr)
    ...
    def run_trial_model(
        ...
        while not self.determine_trial_finished(attempt_limit):
            ...
            while num_steps<action_limit:
                ...
                # convert idx to Action object (idx -> str -> Action)
                action = self.env.action_map[self.env.action_space[action_idx]]
                next_state, reward= self.world(action, state)
                num_steps +=1
                mask = 0 if num_steps>=action_limit else 1
                self.simulated_memory.push(state, action_idx, mask, next_state, reward)
    ...
    def finish_subject(self, strategy="A2C", transfer_strategy="A2C"):
        super(A2CAgent, self).finish_subject(strategy, transfer_strategy, self)
